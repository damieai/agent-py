...
type Task = {id: string; status: string; result: string | null; waiting_reason: string | null; contract: {kind: string; goal: string; resource: string}; operations?: Operation[]; approvals?: Approval[]; artifacts?: {id: string; kind: string; digest: string}[]; spent_micro_usd: number};
...
function App() {
  ...
  async function create() {
    await mutate(async () => {
      const created = await request('/api/v1/tasks', 'POST', {kind, goal, project: 'demo', environment: 'lab', resource: `demo-${crypto.randomUUID()}`}, crypto.randomUUID());
  ...
  return <main>
    ...
    <div className="grid"><aside><section className="panel"><h2>新建任务</h2><label htmlFor="kind">任务路径</label><select id="kind" value={kind} onChange={e => setKind(e.target.value)}><option value="repair">研发 · 修复与交付</option><option value="incident">运维 · 调查与恢复</option></select><label htmlFor="goal">目标</label><textarea id="goal" rows={4} value={goal} onChange={e => setGoal(e.target.value)}/><button className="primary" disabled={!token || busy || goal.length < 5} onClick={() => void create()}>提交任务</button><p className="hint">任务由持久化 Worker 推进。高影响动作等待批准。</p></section>
      ...
      <article className="panel details">{!task ? <div className="empty"><h2>选择一个任务</h2><p>查看执行记录、审批对象与验证产物。</p></div> : <>
        ...
        <div className="actions"><button disabled={busy || task.status === 'TERMINATED'} onClick={() => void mutate(async () => {await request(`/api/v1/tasks/${task.id}/cancel`, 'POST');})}>取消后续执行</button><button disabled={busy || task.status === 'TERMINATED'} onClick={() => void mutate(async () => {await request(`/api/v1/tasks/${task.id}/takeover`, 'POST');})}>人工接管</button></div>
