...
function App() {
  ...
  const changeSession = useCallback((nextToken: string, message = '') => {
    sessionEpoch.current++;
    authenticated.current = false;
    refreshSequence.current++;
    current.current = {token: nextToken, selected: null};
    setToken(nextToken); setSelected(null); setTask(null); setTasks([]);
    setShowOps(false); setBusy(false); setError(message);
  }, []);
  const renderedEpoch = sessionEpoch.current;
  const accessLost = useCallback(() => {
    if (sessionEpoch.current === renderedEpoch) changeSession('', '访问已失效，请重新输入有效凭证');
  }, [changeSession, renderedEpoch]);
  ...
  async function request(path: string, method = 'GET', body?: unknown, key?: string) {
    ...
    if (epoch !== sessionEpoch.current) throw new DOMException('访问身份已变更', 'AbortError');
    if (!response.ok) throw Object.assign(new Error(data.error?.message || JSON.stringify(data.detail || data)), {status: response.status});
    return data;
  }
  async function refresh() {
    ...
    try {
      ...
      setTask(detail);
      setError('');
    } catch (e) { if (isCurrent()) {
      if (authenticated.current && [401, 403].includes((e as {status?: number}).status || 0)) accessLost();
      else { setTask(null); setTasks([]); setError(String(e)); }
    } }
  }
  ...
  async function mutate(action: () => Promise<void>) {
    const epoch = sessionEpoch.current;
    setBusy(true);
    try { await action(); if (epoch === sessionEpoch.current) await refresh(); }
    catch (e) { if (epoch === sessionEpoch.current) setError(String(e)); }
    finally { if (epoch === sessionEpoch.current) setBusy(false); }
  }
  async function create() {
    await mutate(async () => {
      const created = await request('/api/v1/tasks', 'POST', {kind, workflow: mode === 'live' ? workflow : 'investigate', goal, project: 'demo', environment: 'lab', resource: mode === 'simulation' ? `demo-${crypto.randomUUID()}` : resource}, crypto.randomUUID());
  ...
  return <main>
    ...
    <section className="access"><label htmlFor="token">访问凭证</label><input id="token" type="password" autoComplete="off" value={token} onChange={e => changeSession(e.target.value)} placeholder="输入短期 Bearer Token；仅保存在当前页面内存"/><button onClick={() => changeSession('')}>清除</button></section>
    ...
    <div className="grid"><aside><section className="panel"><h2>新建任务</h2><label htmlFor="kind">任务路径</label><select id="kind" value={kind} onChange={e => {setKind(e.target.value); if (e.target.value === 'incident' && workflow === 'repair_candidate') setWorkflow('investigate');}}><option value="repair">研发 · 修复与交付</option><option value="incident">运维 · 调查与恢复</option></select><label htmlFor="workflow">执行范围</label><select id="workflow" value={workflow} disabled={mode !== 'live'} onChange={e => setWorkflow(e.target.value)}><option value="investigate">调查与人工审阅</option><option value="investigation_loop">多轮只读调查（最多三轮）</option>{kind === 'repair' && <option value="repair_candidate">生成候选补丁并验证</option>}</select>{mode === 'live' && <><label htmlFor="resource">授权资源</label><input id="resource" value={resource} onChange={e => setResource(e.target.value)} placeholder="与运维配置的资源名一致"/></>}<label htmlFor="goal">目标</label><textarea id="goal" rows={4} value={goal} onChange={e => setGoal(e.target.value)}/><button className="primary" disabled={!token || busy || goal.length < 5} onClick={() => void create()}>提交任务</button><p className="hint">任务由持久化 Worker 推进。高影响动作等待批准。</p></section>
      ...
      <article className="panel details">{!task ? <div className="empty"><h2>选择一个任务</h2><p>查看执行记录、审批对象与验证产物。</p></div> : <>
        ...
        {task.contract.workflow === 'investigation_loop' && <InvestigationPanel key={`investigation:${task.id}:${token}`} taskId={task.id} token={token} onAccessLost={accessLost}/>}
        ...
        <EventPanel key={`events:${task.id}:${token}`} taskId={task.id} token={token} onAccessLost={accessLost}/>
