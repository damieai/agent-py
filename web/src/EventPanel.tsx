...
export function EventPanel({taskId, token}: {taskId: string; token: string}) {
    ...
    async function connect() {
      ...
      try {
        ...
        await consumeStream(response, controller.signal, event => {
          if (event.type === 'access_revoked') { stopped = true; setItems([]); setError('凭证过期或授权已撤销'); setStatus('已停止'); return; }
