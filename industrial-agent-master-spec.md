...
版本：1.1 · 更新日期：2026-09-19 · 定位：生产工程与高阶 Agent 面试项目
...
## 参考项目：企业订单异常处理与履约 Agent

**项目定位：面向运营人员的多租户 Agent 服务，能够调查订单异常、检索政策、提出方案、申请授权、执行变更并持续核实结果。**这是本规格建议的首条业务主线，可替换行业背景；涉及真实退款、库存修改和通知发送时，必须先完成相应授权与故障验收。

选择这一场景，是因为同一条链路能自然覆盖结构化查询、政策检索、附件解析、跨系统写入、长任务、人工审批、并发约束和对账；高级能力有共同的业务目标，便于比较不同实现。

### 任务族与业务规则

| 任务族 | 输入与交付 | 可展示的能力 | 明确的不变量 |
|---|---|---|---|
| T1 异常调查 | 订单号 → 原因、时间线和证据 | SQL 语义层、Hybrid RAG、引用验证 | 数据只来自当前租户和授权对象 |
| T2 方案准备 | 用户约束 → 补发/退款候选方案 | 规划、证据时效、风险判断 | 准备方案不产生真实写操作 |
| T3 受控执行 | 已授权方案 → 业务回执 | 审批、Action Ledger、Saga、幂等 | 累计退款不超过可退额；库存不为负 |
| T4 中断恢复 | 断连、崩溃或迟到回调 → 已核实结果 | 持久化工作流、UNKNOWN 对账、fencing | 未确认动作不被新身份盲目重做 |
| T5 多订单调查 | 异常清单 → 分组分析与逐项处置建议 | 程序化工具、多 Agent、预算调度 | 每项对象独立授权；批次结果可部分完成 |
| T6 政策与附件冲突 | 新旧制度、截图、恶意文档 → 可解释结论 | 多模态、时态检索、注入防御 | 外部内容不能改变执行权限 |

退款金额使用精确十进制或按币种约定的最小单位；记录税费、部分退款和多次退款的业务规则。演示渠道提供可配置的幂等期限和查询滞后，明确它是仿真服务；接入真实渠道后必须重新验证保证。

### 一条贯穿式演示

用户请求“核实这批订单的延误原因，按当前政策准备处置；超出已授权范围的操作交给我批准”。系统识别主体和范围，查询订单与履约事件，检索适用政策，输出证据及方案；满足现有授权的动作直接推进，需要审批的动作绑定具体对象和参数。

演示中在外部渠道提交成功、Worker 尚未保存结果时注入崩溃。重启后，系统使用原 operation_id 查询或按渠道契约原键重试；若暂时查不到则保持 UNKNOWN。随后模拟用户取消和迟到成功回调，系统停止新派发、保存已确认副作用并按规则决定是否需要补偿；最终页面能展示动作时间线、审批依据、对账结果和费用。

第二轮加入政策撤回、跨租户文档和恶意工具响应，验证权限和证据失效；第三轮只替换上下文策略或规划策略，在同一任务集展示质量、成本与时延变化。

### 默认技术路线与取舍

以下是开工基线，不是对任意规模的通用推荐；依赖版本在实施时锁定，并验证选定组合。仓库目录仅表示拟交付结构，当前文档不代表这些模块已实现。

| 责任 | 初始选择 | 升级或替换条件 |
|---|---|---|
| API 与领域模型 | Python、FastAPI、Pydantic；业务规则独立模块 | 基于并发、类型契约与适配器需求调整 |
| 长任务与恢复 | Temporal + Python Worker | 如采用其他引擎，必须实现等价故障验收 |
| Agent 策略 | 显式循环 + 类型化计划 | 需要复杂局部图时评估 LangGraph，明确恢复归属 |
| 业务与动作存储 | PostgreSQL，显式事务与迁移 | 按观测到的负载拆分，避免一开始分库 |
| 检索 | PostgreSQL 结构化查询 + 向量检索；独立检索接口 | 中文分词、关键词召回或规模不满足时增加专用搜索引擎 |
| 产物与录制包 | S3 兼容对象存储 | 依据大小、地域和保留要求配置 |
| 缓存与速率限制 | 需要时使用 Redis | 不用 Redis 代替动作持久记录或第三方幂等 |
| 观测与评测 | OTel + 指标/Trace 后端 + 自有任务评测器 | 按调试与运营需求接入评测平台，保持数据可导出 |
| 沙盒 | 独立受限执行器；本地先验证隔离边界 | 不可信多租户代码按威胁模型评估 MicroVM |
| 部署 | 容器化、本地可重复启动；生产明确持久存储和恢复方案 | 多节点调度和运维条件成熟后引入 Kubernetes |

推荐代码按 `domain/`、`runtime/`、`policies/`、`context/`、`adapters/`、`api/`、`evals/`、`tests/faults/` 和 `ops/` 分区；框架对象与供应商 SDK 留在适配层。每项选型有 ADR，至少说明一个替代方案、取舍原因、失败边界和退出成本。
...
| 增量 | 对应阶段 | 完成条件 |
|---|---|---|
| A. 可测量基线 | 0—1 | T1/T2；单 Agent、少量工具、ContextBundle、任务评测与基础 Trace |
| B. 可靠业务闭环 | 2 | T3/T4；授权审批、动作账本、Temporal、UNKNOWN 对账、崩溃和竞态测试 |
| C. 知识与规模化任务 | 3 | T5/T6；增量数据、时态证据、程序化查询、上下文与检索消融 |
