...
尚未验证：真实付费模型、企业账号与外部写入、Docker 沙盒、Compose 容器组合、生产场景与多浏览器端到端、完整系统形式化证明、负载/SLO、灾备恢复与 GPU 训练。当前运行环境没有可用的 Docker 引擎，代码执行器保持关闭，不降级到宿主机执行。
...
发布版本固定里程碑新增 34 项默认测试和 1 项 Temporal 集成测试。全量 396 通过、5 跳过（24.13 秒，正常退出），静态检查通过。覆盖任务摘要绑定、原请求幂等、未固定 Worker 拒绝固定任务、代码/评测/绑定摘要不匹配、配置漂移、实时 Grant/停用/取消/接管、跨版本旧动作对账、发布过滤先于批量上限、审计三处身份一致、文件类型/大小/权限/禁止覆盖、离线 CLI、Service 重建和取消恢复。首次全量与其他验证并发运行时，虽全部断言完成，进程退出触及 90 秒限制；已单独重跑确认正常完成。

原生 PostgreSQL 的现有迁移/RLS/约束/并发测试及新增 JSON 发布过滤通过（1 项，12.42 秒）。真实本地 Temporal 验证摘要专属队列、Outbox 派发、Service 重建及接管后取消完成（1 项，21.71 秒）；这是进程内重建配合实际 Temporal 服务，不是整机崩溃或完整双版本排空演练。新测试首次启动缺少 await，修正后通过。

重新执行隔离评测门禁 PASS，并以实际报告运行 `release-build` / `release-check`，构建和预检得到相同 `sha256:c4ec9e46115347b3168aae37bf24e5812a4412f5e49ee527afcb806b79846618`，清单 67,023 字节，保存在本机 `.runtime/release-smoke-z4gvpsn0/release.json`。清单包含运行源码/锁文件摘要、配置和原始评测证据；生成过程未连接模型或企业接口。该 ID 是本轮本地证据的结果，不是生产发布批准或稳定示例常量。

本轮无数据库结构或前端变更；CI 已增加发布清单构建，但未执行远端流水线。发布签名、可信构建证明、安装依赖/应用镜像实物校验、自动旧 Worker 排空和真实灰度仍待实现。启用前必须升级或停止不识别版本的旧 Dispatcher，详见[发布说明](releases.md)。

发布签名里程碑新增 45 项默认测试，全量 441 通过、5 跳过（27.88 秒），静态检查通过。使用真实 Ed25519 签名覆盖声明篡改、审计签名域混用、独立发布 pin、未知/撤销/替换公钥、audience/运行环境限制、未来签发与过期边界、最长寿命、重复字段/key_id、私钥权限与文件读取限制、无签名降级拒绝、同版本续签换钥、运行中到期复核、readiness 失效、撤钥保留 Outbox 和 UNKNOWN 继续对账。现有发布版本固定、审计签名和授权撤销测试继续通过。

隔离评测门禁再次 PASS。用实际报告构建新发布 `sha256:744abbb1505d49edd90eb29b4c052b47a3bf3fb2a107b0b4b213edafe8f8235e`，在 `.runtime/signed-release-smoke-fsx11oq_/` 生成本地测试密钥、签发及离线验签，然后带签名配置执行 release-check，结果通过；将测试 key 标记 revoked 后，同一预检以 RELEASE_SIGNATURE_INVALID 和退出码 1 拒绝。测试密钥和清单仅保留在忽略目录，未配置生产密钥或访问企业接口。

本轮没有数据库结构、Workflow 或前端变更，未重复 PostgreSQL/Temporal/前端构建；默认测试中的 Dispatcher 使用 Mock 客户端，签名本身是实际密码学验证。尚未验证生产密钥分发、全副本撤销时延、KMS/HSM、自动续签、可信构建证明或真实灰度发布。签名要求默认可选，需由部署准入统一强制；不能据此宣称对已发出的请求即时撤销。部署步骤见[发布签名](release-signing.md)。

工作台浏览器里程碑锁定 Playwright 1.63.0，使用 Chromium 153.0.8010.12（revision 1243）、生产前端构建及真实临时 FastAPI/SQLite，七项 E2E 全部通过（最终复验 25.6 秒，无重试）。覆盖开发者批准拒绝、reviewer 两次批准与 SUCCESS、SSE 终止、真实审计下载、接管/版本恢复/取消、授权证据预览、Grant 撤销清屏、输入框清空、迟到创建和清除后重新输入相同 Token 的迟到下载隔离，以及不完整凭证继续编辑。外部效果来自持久仿真，Activity 由测试控制接口推进，没有启动 Temporal 或调用企业系统。

旧生产构建的清空凭证用例实际失败：清空输入后仍有 1 个任务条目，10 秒断言超时。页面已用身份代次统一清理和隔离异步结果；修复后全部七项通过。初次浏览器启动暴露缺少系统运行库的问题，本机将 libnspr4/libnss3/libasound2t64 解压至项目忽略目录，并仅为测试进程设置加载路径。启动探测改为服务实际绑定后的就绪信号，避免测试环境中未监听端口探测停滞。

应用与 E2E 的严格 TypeScript 检查、Vite 生产构建、七项 Node SSE 单元测试及 Python 静态检查通过。默认后端套件 441 通过、5 跳过（36.40 秒）。Bitbucket 已配置浏览器依赖安装和报告归档，远端流水线未运行。本轮无数据库结构或生产后端协议变更，未重复 PostgreSQL/Temporal 集成；多浏览器、移动布局、完整无障碍、真实模型修复界面和企业写闭环仍未验收。说明与复现见[浏览器测试](browser-testing.md)。
