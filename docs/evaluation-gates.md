...
## Langfuse 实验与门禁扩展（planned）

[Langfuse 方案](langfuse.md) 的 LF-02/LF-04 将增加真实模型任务评测。现有命令和离线门禁继续按当前格式工作；新 schema、实验 runner 及平台适配器尚未实现。

实验平台用于轨迹分析、样本标注与候选比较；正式门禁消费冻结的本地数据集、逐案例结果、评分器版本、供应商 usage/价格版本和阶段耗时，按预先固定策略重算。不能仅依据平台平均分通过，也不能把旧仿真案例数当作真实模型样本数。

新证据至少包括单次/多轮调查、检索策略及有界反馈修复的配对对照。质量、费用、时延分别报告，失败任务和 judge 费用计入总预算；UNKNOWN usage、缺失案例、无效评分、不可比较版本及样本不足返回证据不足。确定性安全门槛不允许被高平均质量抵消；LLM judge 需人工校准，不能覆盖独立验证失败。

实验可以在平台不可用时先保存本地原始结果，后续补上传；原始证据不足不能靠 trace 补齐或静默重发模型。CI 继续使用本仓库已有流水线，新增显式预算的独立模型评测入口，不让默认单元测试依赖平台或产生付费调用。发布固定扩展见 [提示词与发布计划](releases.md)。
