...
def issue_dev_token(settings: Settings, principal: Principal, seconds: int = 3600) -> str:
    ...
    return jwt.encode({**principal.model_dump(), "sub": principal.subject,
                       "iss": settings.auth_issuer, "aud": settings.auth_audience,
                       "iat": datetime.now(UTC), "exp": datetime.now(UTC) + timedelta(seconds=seconds)},
...
def authenticate(settings: Settings, token: str) -> Principal:
    ...
    try:
        claims = jwt.decode(token, key, algorithms=["RS256" if settings.auth_public_key else "HS256"],
                            issuer=settings.auth_issuer, audience=settings.auth_audience,
                            options={"require": ["exp", "iat", "sub", "iss", "aud"]})
        return Principal(tenant_id=claims["tenant_id"], subject=claims["sub"],
                         roles=claims["roles"], projects=claims["projects"],
...
def check_grant(s, tenant: str, subject: str, project: str, environment: str):
    grant = s.scalar(select(Grant).where(Grant.tenant_id == tenant, Grant.subject == subject,
                                       Grant.project == project, Grant.environment == environment))
...
def authorize(s, principal: Principal, task: Task, role: str | None = None):
    c = task.contract
    if task.tenant_id != principal.tenant_id or c["project"] not in principal.projects or c["environment"] not in principal.environments:
