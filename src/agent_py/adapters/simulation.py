...
class SimulatedSystem:
    ...
    def inject(self, tenant: str, operation: str, mode: str, remaining: int = 1):
        ...
        with self.connect() as c:
            c.execute("INSERT OR REPLACE INTO faults VALUES (?, ?, ?, ?)",
                      (tenant, operation, mode, remaining))

    def snapshot(self, tenant: str, resource: str) -> dict:
        with self.connect() as c:
            row = c.execute("SELECT body FROM resources WHERE tenant=? AND resource=?",
                            (tenant, resource)).fetchone()
            return json.loads(row[0]) if row else {
                "revision": 1, "candidate_sha": digest("fixed-candidate"),
                "base_sha": digest("baseline"), "image_digest": "sha256:" + digest("healthy"),
                "replicas": 2, "healthy": False, "effect_count": 0,
            }

    def query(self, tenant: str, operation: str) -> dict | None:
        with self.connect() as c:
            fault = c.execute("SELECT * FROM faults WHERE tenant=? AND operation=?",
                              (tenant, operation)).fetchone()
            if fault and fault["mode"] == "query_hidden" and fault["remaining"] > 0:
                c.execute("UPDATE faults SET remaining=remaining-1 WHERE tenant=? AND operation=?",
                          (tenant, operation))
                return None
            row = c.execute("SELECT result FROM receipts WHERE tenant=? AND operation=?",
                            (tenant, operation)).fetchone()
    ...
    def execute(self, tenant: str, operation: str, tool: str, resource: str, params: dict) -> dict:
        ...
        with self.connect() as c:
            ...
            existing = c.execute("SELECT * FROM receipts WHERE tenant=? AND operation=?",
                                 (tenant, operation)).fetchone()
            if existing:
            ...
            if tool == "merge_pr" and (params["source_sha"] != state["candidate_sha"] or params["target_sha"] != state["base_sha"]):
            ...
            result = {"external_id": "sim-" + operation, "confirmed": True,
                      "resource": resource, "state": state, "simulation": True}
...
class DisabledLiveExecutor:
    ...
    def execute(self, *args, **kwargs):
        raise DomainError("LIVE_WRITES_DISABLED", "Live write capability has not been certified", 503)

    def query(self, *args, **kwargs):
        raise DomainError("LIVE_RECONCILIATION_UNCONFIGURED", "Configure the authoritative query adapter", 503)
